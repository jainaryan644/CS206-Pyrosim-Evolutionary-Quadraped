...
sim = pyrosim.Simulator( play_paused = True, eval_time =1000) #starts the simulation paused
...
SN0 = sim.send_sensor_neuron( sensor_id = T0 )  # ground neuron
SN1 = sim.send_sensor_neuron( sensor_id = T1 ) # tip of red cylinder neuron
...
sim.send_synapse(source_neuron_id = SN1 , target_neuron_id = MN2 , weight = -1.0 ) #connects SN1 & MN2
sim.start()
sim.wait_to_finish()
